...
class L2PGDAttack:
  def __init__(self, model, epsilon, num_steps, step_size, random_start, loss_func):
    ...
    self.sigma = 100.0/255
    self.E_steps = 1  #number of iterations for expectation
  ...
  def perturb(self, x_nat, y, sess):
    ...
    for i in range(self.num_steps):
      ...
      # print(x.shape)
      # cov = (self.sigma**2)*np.identity(32*32*3)

      for j in range(self.E_steps):
        d = np.zeros((32*32*3))
        for j in range(32*32*3):
          d[j] = np.random.normal(scale=self.sigma)
        # d = np.random.multivariate_normal(mean,cov).reshape(32,32,3)
        d = d.reshape(32,32,3)
        grad = grad + sess.run(self.grad, feed_dict={self.model.x_input: x+d,
                                                    self.model.y_input: y})
